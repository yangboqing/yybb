...
- (void)viewDidAppear:(BOOL)animated{
    ...
    [nav cancelRightPanGestureFunction];
...
- (void)viewDidLoad {
    ...
    backSubview.frame = CGRectMake((self.view.frame.size.width/2 - 60)/2, MainScreen_Height - 72.0, 60, 60);
    downloadSubview.frame = CGRectMake((self.view.frame.size.width/2 - 60)/2 + self.view.frame.size.width/2, MainScreen_Height - 72.0, 60, 60);
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if(ALAuthorizationStatusDenied == author){ //用户拒绝
        ...
        UIAlertView *tmpAlertView = [[UIAlertView alloc] initWithTitle:@"访问相册失败"
                                                               message:@"请在\"设置-->隐私-->照片\"中\"快用助手\"对应的开关调为打开状态，再次尝试保存即可"
                                                              delegate:nil
