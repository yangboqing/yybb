...
#pragma clang diagnostic pop
        ...
        {
            _toPoint = CGPointMake((_toPoint.x - _startPoint.x)*0.8 + _startPoint.x, (_toPoint.y - _startPoint.y)*0.8 + _startPoint.y);
...
- (void)scaling
{
    if (_state == SRSlimeStateShortening) {
        self.toPoint = CGPointMake((_toPoint.x - _startPoint.x)*0.9 + _startPoint.x, (_toPoint.y - _startPoint.y)*0.9 + _startPoint.y);
        float p = distansBetween(_startPoint, _toPoint) / _viscous;
